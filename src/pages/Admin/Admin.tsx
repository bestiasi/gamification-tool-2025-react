...
function Admin() {
  ...
  const [generatedTransferLink, setGeneratedTransferLink] = useState('');
  ...
  }, [requests, selectedDepartment, searchQuery]);
  ...
  const filterRequests = () => {
    ...
    // Search by user name or task name
    if (searchQuery.trim()) {
      const query = searchQuery.toLowerCase();
      filtered = filtered.filter(req => 
        req.userName.toLowerCase().includes(query) ||
        req.task.toLowerCase().includes(query)
      );
    }
  ...
  const handleReview = async (requestId: string, newStatus: 'approved' | 'rejected') => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'pointRequests', requestId), updateData);
  ...
  const handleTransferAdmin = async (e: React.FormEvent) => {
    ...
    if (newAdminEmail === user?.email) {
      setTransferError('Nu poți transfera către tine însuți');
      return;
    }
    ...
    try {
      ...
      setGeneratedTransferLink(acceptLink);
      setTransferMessage(`✅ Link de transfer generat cu succes! Copiază și trimite link-ul către ${newAdminEmail}`);
      setNewAdminEmail('');
  ...
  const handleAddSecretary = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'secretaries', secretaryEmail), {
            ...
            <div className="tab-content">
              <div className="admin-filters">
                ...
                <div className="filter-group">
                  ...
                  <input
                    type="text"
                    placeholder="Nume utilizator sau task..."
                    value={searchQuery}
              ...
              <div className="requests-list">
                {filteredRequests.length === 0 ? (
                  ...
                  filteredRequests.map(request => {
                    ...
                    const isReviewing = reviewingRequest === request.id;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="request-body">
                          <p className="request-task">{request.task}</p>
                          {!isExpanded && (
                            <>
                              {request.taskNumber && (
                                <p className="request-number">{request.taskNumber}x</p>
                              )}
                              {request.eventDate && (
                                <p className="request-event-date">📅 {new Date(request.eventDate).toLocaleDateString('ro-RO')}</p>
                              )}
                            </>
                          )}
                          ...
                          {isExpanded && (
                            <>
                              ...
                              {request.eventDate && (
                                <p className="request-event-date"><strong>Data Eveniment:</strong> {new Date(request.eventDate).toLocaleDateString('ro-RO')}</p>
                              )}
                              {request.proofUrl && (
                              ...
                              {request.details && (
                                <p className="request-details-full"><strong>Detalii:</strong> {request.details}</p>
                              )}
                        ...
                        {/* Quick action buttons for pending requests - always visible */}
                        {selectedStatus === 'pending' && !isReviewing && (
                          <div className="quick-actions" onClick={(e) => e.stopPropagation()}>
                            <button
                              ...
                              onClick={() => {
                                ...
                                setReviewComment('');
                                if (!isExpanded) setExpandedCard(request.id);
                              }}
                          ...
                          <div className="review-section" onClick={(e) => e.stopPropagation()}>
                            <div className="review-comment-form">
                              <label className="review-label">
                                💬 Comentariu (opțional pentru aprobare, obligatoriu pentru respingere):
                              </label>
                              <textarea
                                className="review-textarea"
                                placeholder="Ex: Bravo! Continuă tot așa! / Cererea nu îndeplinește criteriile..."
                                value={reviewComment}
                        ...
                        {selectedStatus !== 'pending' && isExpanded && (
                          <>
                            {request.reviewedBy && (
                            ...
                            {request.adminComment && (
                              <div className={`admin-comment ${selectedStatus}`}>
              ...
              <form onSubmit={handleTransferAdmin} className="transfer-form">
                ...
                {transferMessage && (
                  <div className="transfer-success">
                    <p style={{ marginBottom: '15px', fontWeight: 'bold' }}>{transferMessage}</p>
                    {generatedTransferLink && (
                      <>
                        ...
                        <button 
                          ...
                          onClick={() => {
                            navigator.clipboard.writeText(generatedTransferLink);
